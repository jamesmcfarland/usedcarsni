...
const main = async () => {
  ...
  const totalCars = await getNumberOfResults(tab, searchStr);
...
const getCarInfo = async (tab, url) => {
  let car = { url };
  await tab.goto(url);
  await tab.waitForSelector(".technical-params");
  const carDatas = await tab.evaluate(() => {
  ...
  const listingName = await tab.evaluate(() => {
  ...
  const price = await tab.evaluate(() => {
  ...
  return bluebird
    .map(
      ...
      async (url, i, totalUrls) => {
        ...
        try {
          const unordered = await getCarInfo(tab, url);
  ...
  const urlsList = await bluebird
    .map(
      ...
      async (pageIndex, i) => {
        let urls = [];
        const tab = await browser.newPage();
        const newUrls = await getSearchResults(tab, pageIndex, searchStr);
...
const getSearchResults = async (tab, pageNo, searchStr) => {
  searchStr += `&pagepc0=${pageNo}`;
  await tab.goto(`${ROOT_URL}/search_results.php?${searchStr}`);
  let urls = await tab.evaluate(() => {
    let returnData = [];
...
const getNumberOfResults = async (tab, searchStr) => {
  await tab.goto(`${ROOT_URL}/search_results.php?${searchStr}`);
  await tab.waitForSelector(".page-control-label");
  const resultCount = await tab.evaluate(() => {
    const data = document
